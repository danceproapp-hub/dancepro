...
export const ja: Dictionary = {
  ...
  home: {
    ...
    ideaPara1:
      "DancePro を開いて、必要な条件そのままで探します。種目、レベル、踊るロール、自分のスタジオの近くでも、世界のどこでも。返ってくるのは実在のダンサーばかりで、それぞれが何を踊り、どんなパートナーを求めているかをすでに示しています。",
    ideaPara2:
      "条件の合う相手にコネクションリクエストを送ります。相手が承認すれば、そのままアプリ内で会話が始まります。週末までには練習スケジュールを決め、一緒に目指す競技会も選んでいるはずです。",
    ideaPara3:
      "それが DancePro です。あなたの条件に合うダンサーがひとつの場所に集まり、しかも全員があなたと同じ目的でここにいます。",
